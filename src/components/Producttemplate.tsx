...
function Producttemplate(): JSX.Element {
  ...
  const [modalSource, setModalSource] = useState<"preview" | "gallery">(
    "preview"
  );
  ...
  const product = useMemo(() => {
    ...
    if (typeof samplesRaw === "string") {
      try {
        samplesRaw = JSON.parse(samplesRaw);
      } catch {
        /* keep string */
      }
    }
    ...
    if (typeof technicalRaw === "string") {
      try {
        technicalRaw = JSON.parse(technicalRaw);
      } catch {
        /* keep string */
      }
    }
    ...
    return {
      ...
      subcategory:
        p.SubCategory ?? p.Subcategory ?? p.sub_category ?? p.SubCategory ?? "",
  ...
  // Cutting samples images from CuttingSamplesURL
  const cuttingSamples = useMemo(() => {
    const raw =
      rawProduct?.CuttingSamplesURL ??
      rawProduct?.Cutting_Samples_URL ??
      rawProduct?.cutting_samples_url ??
      rawProduct?.cuttingSamplesUrl ??
      rawProduct?.CuttingSamples ??
      null;
    if (!raw) return [];
  ...
  const technicalTable = useMemo(() => {
    ...
    if (
      ...
      const headers = Array.from(
        new Set(parsed.flatMap((o: any) => Object.keys(o)))
      );
    ...
    if (Array.isArray(parsed)) {
      return {
        type: "kv" as const,
        rows: parsed.map((v: any, i: number) => [`Item ${i + 1}`, String(v)]),
      };
    }
    ...
    useMemo(() => {
      try {
        ...
        return Object.entries(parsed)
          ...
          .map(([key, val]) => ({ title: key, content: String(val) }));
      } catch {
        return [];
      }
  ...
  const videoEmbed = useMemo(
    () => getYouTubeEmbedUrl(product?.rawVideo),
    [product?.rawVideo]
  );
  ...
  const tabs = useMemo(() => {
    ...
    if (descriptionSections.length > 0)
      t.push({ id: "description", label: "Description" });
    if (technicalTable !== null)
      t.push({ id: "technical", label: "Technical Specification" });
    if (product?.rawVideo) t.push({ id: "video", label: "Video" });
    if (galleryImages.length > 0)
      t.push({ id: "samples", label: "Cutting Sample" });
    return t;
  }, [
    descriptionSections,
    technicalTable,
  ...
  const activeCount = useCallback(
    (source: "preview" | "gallery") =>
      source === "preview" ? previewImages.length : galleryImages.length,
  ...
  // Touch handlers to enable swipe navigation
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white text-gray-900">
      ...
      >
        <div
          className="w-64 bg-white overflow-hidden text-sm text-gray-900 shadow-lg border border-gray-100 transition-shadow hover:shadow-xl"
        ...
        >
          <div className="h-14 flex items-center justify-between px-5 bg-gradient-to-r from-gray-50 to-white border-b border-gray-100">
            <div
              className="text-xs font-bold tracking-wider text-whiteBgButtonBg"
              ...
              {relatedProducts.map((rp: any) => (
                ...
                >
                  <div
                    ...
                    className="group-hover:opacity-100 bg-whiteBgTextHover transition-opacity"
                  />
                  <div className="flex-1 min-w-0">
                    <div
                      className="text-sm font-medium text-gray-800 group-hover:text-gray-900 transition-colors"
      ...
      <div
        ref={topRef}
        className="mx-auto px-5 sm:px-8 pt-24 sm:pt-32 pb-20 lg:ml-80 lg:pl-8 lg:pr-8 xl:pr-16"
      ...
      >
        ...
        <div className="flex items-center gap-3 text-sm mb-8 text-gray-600">
          ...
          <span className="text-gray-900 font-medium font-secondary">
            {product.name}
          </span>
        </div>
...
*/}
          ...
          <div className="space-y-5 w-full max-w-full overflow-hidden">
              ...
              {previewImages.length > 1 && (
                <>
                  <button
                    ...
                    aria-label="previous"
                    className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 w-9 h-9 sm:w-11 sm:h-11 bg-white/95 backdrop-blur-sm border border-gray-200 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all hover:bg-white hover:scale-110 shadow-lg"
                  >
                  ...
                  <button
                    ...
                    aria-label="next"
                    className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 w-9 h-9 sm:w-11 sm:h-11 bg-white/95 backdrop-blur-sm border border-gray-200 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all hover:bg-white hover:scale-110 shadow-lg"
                  >
            ...
            {/* Horizontal scrolling thumbnail preview */}
            <div
              className="w-full mx-auto overflow-x-auto overflow-y-hidden px-4"
              style={{
                maxWidth: 700,
                scrollbarWidth: "none",
              ...
              <div className="flex gap-2 md:gap-3 min-w-0">
                ...
                {previewImages.map((img, idx) => (
                  <button
                    ...
                    style={{
                      borderWidth: 2,
                      borderColor: idx === previewIndex ? "#6b0f0f" : "#e5e7eb",
                    }}
          ...
          <div className="flex flex-col space-y-6">
            ...
            {features && features.length > 0 && (
              <div className="p-6 border border-gray-200 shadow-sm">
                <h3 className="text-sm uppercase text-gray-600 mb-4 font-bold tracking-wider">
                  Key Features
                </h3>
                <ul className="grid grid-cols-1 gap-3">
                  {features.map((f: string, i: number) => (
                    <li
                      key={i}
                      className="flex items-start gap-3 text-sm text-gray-800 group"
                    >
        ...
        {/* Sticky section nav */}
        <div
          className="w-full bg-white/40 backdrop-blur-md border-y border-gray-200 z-30 mb-10 shadow-sm"
          ...
          {descriptionSections.length > 0 && (
            ...
            >
              <div className="p-8 border-b border-gray-200">
                <h2 className="text-4xl md:text-4xl font-medium text-gray-900 mb-2">
                  Product Description
                </h2>
                <p className="text-gray-600">
                  Comprehensive details about {product.name}
                </p>
              </div>
              <div className="p-8">
                  ...
                  {descriptionSections.map((section, idx) => (
                    <div key={idx} className="space-y-3">
                      <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-2xl bg-gray-50 p-2 rounded-lg font-medium text-gray-900">
                          {section.title}
                        </h3>
                      </div>
                      ...
                      <thead>
                        <tr className="bg-gray-900 text-white">
                          ...
                          {technicalTable.rows.map((_, idx) => (
                            <th
                              key={idx}
                              className="py-4 px-5 text-left font-semibold uppercase text-xs tracking-wider border border-gray-700"
                            >
                              Option {idx + 1}
                            </th>
                          ))}
                        ...
                        {technicalTable.headers.map((header, hIdx) => (
                          ...
                          >
                            ...
                            {technicalTable.rows.map((row, rIdx) => (
                              <td
                                key={rIdx}
                                className="py-4 px-5 text-sm text-gray-900 whitespace-pre-wrap border border-gray-200"
                              >
                                {row[hIdx]}
                              </td>
                            ))}
                    ...
                    <table className="w-full text-sm">
                      <tbody>
                        {technicalTable.rows.map(([k, v], i) => (
                          ...
                          >
                            <td
                              className="py-4 pr-8 align-top text-sm text-gray-700 uppercase tracking-wider font-bold"
                              style={{ width: "40%" }}
                            >
                              {k}
                            </td>
                            <td className="py-4 pl-8 text-sm font-semibold text-gray-900 whitespace-pre-wrap">
                              {v}
                            </td>
                          </tr>
                        ))}
      ...
      {relatedProducts && relatedProducts.length > 1 && (
        <div
          className="lg:hidden bg-white border-t border-gray-200 shadow-sm mx-5 mb-8"
          style={{ borderRadius: "4px" }}
        >
          <div className="p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-md uppercase text-gray-700 font-medium tracking-wider">
                Related Products
              </h3>
