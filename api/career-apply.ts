...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { name, email, phone, company, resumeLink, cover, position, department } = body;
...
<head>
  ...
  <title>Career Application — ${position}</title>
</head>
...
<table width="620" cellpadding="0" cellspacing="0" border="0"
      ...
      <table width="100%" cellpadding="0" cellspacing="0" border="0"
        ...
        <tr>
          ...
          <td style="padding:15px 18px;vertical-align:top;" width="33%">
            <p style="margin:0 0 4px;font-size:9px;font-weight:700;color:#bbb;
                       letter-spacing:1.5px;text-transform:uppercase;">Current Employer</p>
  ...
  <tr>
    <td style="padding:0 40px 24px;">
      <table width="100%" cellpadding="0" cellspacing="0" border="0"
        ...
        <tr>
          <td style="padding:16px 20px;border-right:1px solid #fde8e8;vertical-align:top;" width="50%">
            <p style="margin:0 0 4px;font-size:9px;font-weight:700;color:#e8111f;
                       letter-spacing:1.5px;text-transform:uppercase;">Position</p>
            <p style="margin:0;font-size:13px;color:#1a1a1a;font-weight:500;line-height:1.4;">${position}</p>
          </td>
          <td style="padding:16px 20px;vertical-align:top;" width="50%">
            <p style="margin:0 0 4px;font-size:9px;font-weight:700;color:#bbb;
                       letter-spacing:1.5px;text-transform:uppercase;">Experience Level</p>
            <p style="margin:0;font-size:13px;color:#1a1a1a;font-weight:500;line-height:1.4;">${department || "—"}</p>
          </td>
        </tr>

        ${resumeLink ? `
        <tr>
          <td colspan="2" style="padding:12px 20px;border-top:1px solid #fde8e8;vertical-align:top;">
            <p style="margin:0 0 4px;font-size:9px;font-weight:700;color:#bbb;
                       letter-spacing:1.5px;text-transform:uppercase;">Resume Link</p>
            <a href="${resumeLink}"
               style="font-size:12.5px;color:#e8111f;text-decoration:none;font-weight:500;
                      word-break:break-all;line-height:1.4;">${resumeLink}</a>
          </td>
        </tr>` : ""}
      </table>
    </td>
        ...
        <tr>
          <td style="padding:16px 20px;">
            <p style="margin:0 0 8px;font-size:9px;font-weight:700;color:#bbb;
                       letter-spacing:1.5px;text-transform:uppercase;">Cover Message</p>
            <p style="margin:0;font-size:13px;color:#444;font-weight:400;
                       line-height:1.7;white-space:pre-wrap;">${cover}</p>
          </td>
        </tr>
      </table>
    </td>
  ...
  <tr>
    <td style="padding:26px 40px 34px;">
      <a href="mailto:${email}?subject=Re%3A%20Your%20Application%20%E2%80%94%20${encodeURIComponent(position)}&body=Hi%20${encodeURIComponent(firstName)}%2C%0A%0AThank%20you%20for%20applying%20for%20the%20${encodeURIComponent(position)}%20role%20at%20Laser%20Technologies.%20"
...
<head>
  <meta charset="UTF-8"/>
  <meta name="viewport" content="width=device-width,initial-scale=1.0"/>
  <title>Application Received — Laser Technologies</title>
</head>
<body style="margin:0;padding:0;background:#f5f5f7;font-family:Helvetica,Arial,sans-serif;">
<table width="100%" cellpadding="0" cellspacing="0" border="0"
  style="background:#f5f5f7;padding:52px 20px;">
  ...
  <tr>
    <td style="padding:38px 40px 30px;">
      ...
      <p style="margin:0 0 14px;font-size:15px;color:#444;line-height:1.85;font-weight:400;">
        ...
        <strong style="color:#0f0f0f;font-weight:600;">${position}</strong> role
        at Laser Technologies. Our HR team will carefully review your application
...
<p style="margin:20px 0 0;font-size:10.5px;color:#ccc;text-align:center;font-weight:400;">
  Laser Technologies Private Limited
    ...
    await transporter.sendMail({
      ...
      replyTo: email,
      subject: `MAIN-WEBSITE [Career Application] ${position} — ${name}`,
      html: adminHtml,
    });
