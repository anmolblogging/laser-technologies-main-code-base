...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    await supabase.from("leads").insert([
      {
        ...
        company,
        subject: event_name,
        message: `Location: ${event_location || 'N/A'} | Date: ${event_date} ${event_end_date ? `to ${event_end_date}` : ''} | Time: ${event_time || 'N/A'}`,
        type: "event",
        website: "MAIN",
      },
    ]);
    ...
    const dateRange = event_end_date
...
<head>
  ...
  <title>Event Registration — ${event_name}</title>
</head>
...
<table width="620" cellpadding="0" cellspacing="0" border="0"
      ...
      <table width="100%" cellpadding="0" cellspacing="0" border="0"
        ...
        <tr>
          <td style="padding:16px 20px;border-right:1px solid #fde8e8;vertical-align:top;" width="50%">
            <p style="margin:0 0 4px;font-size:9px;font-weight:700;color:#e8111f;
                       letter-spacing:1.5px;text-transform:uppercase;">Event</p>
            <p style="margin:0;font-size:13px;color:#1a1a1a;font-weight:500;line-height:1.4;">${event_name}</p>
          </td>
          <td style="padding:16px 20px;vertical-align:top;" width="50%">
            <p style="margin:0 0 4px;font-size:9px;font-weight:700;color:#bbb;
                       letter-spacing:1.5px;text-transform:uppercase;">Date${event_time ? " & Time" : ""}</p>
            <p style="margin:0;font-size:13px;color:#1a1a1a;font-weight:500;line-height:1.4;">
              ${dateRange}${event_time ? `<br/><span style="color:#888;font-size:12px;font-weight:400;">${event_time}</span>` : ""}
            </p>
          </td>
        </tr>
        ${event_location ? `
        <tr>
          <td colspan="2" style="padding:12px 20px;border-top:1px solid #fde8e8;vertical-align:top;">
            <p style="margin:0 0 4px;font-size:9px;font-weight:700;color:#bbb;
                       letter-spacing:1.5px;text-transform:uppercase;">Location</p>
            <p style="margin:0;font-size:13px;color:#1a1a1a;font-weight:500;line-height:1.4;">${event_location}</p>
          </td>
        </tr>` : ""}
      </table>
    </td>
  ...
  <tr>
    <td style="padding:26px 40px 34px;">
      <a href="mailto:${email}?subject=Re%3A%20Your%20Registration%20%E2%80%94%20${encodeURIComponent(event_name)}&body=Hi%20${encodeURIComponent(firstName)}%2C%0A%0AThank%20you%20for%20registering%20for%20${encodeURIComponent(event_name)}.%20"
...
<head>
  <meta charset="UTF-8"/>
  <meta name="viewport" content="width=device-width,initial-scale=1.0"/>
  <title>Registration Confirmed — Laser Technologies</title>
</head>
<body style="margin:0;padding:0;background:#f5f5f7;font-family:Helvetica,Arial,sans-serif;">
<table width="100%" cellpadding="0" cellspacing="0" border="0"
  style="background:#f5f5f7;padding:52px 20px;">
...
<table width="560" cellpadding="0" cellspacing="0" border="0"
  ...
  <!-- Logo header (no badge for auto-reply) -->
  <tr>
    <td style="padding:26px 40px 22px;border-bottom:1px solid #f2f2f2;">
      <p style="margin:0;font-size:14px;font-weight:700;color:#0f0f0f;
                 letter-spacing:4px;text-transform:uppercase;line-height:1;">
  ...
  <tr>
    <td style="padding:38px 40px 30px;">
      ...
      <p style="margin:0 0 14px;font-size:15px;color:#444;line-height:1.85;font-weight:400;">
        ...
        <strong style="color:#0f0f0f;font-weight:600;">${event_name}</strong>.
        Our team will be in touch to confirm your attendance.
      </p>
...
<p style="margin:20px 0 0;font-size:10.5px;color:#ccc;text-align:center;font-weight:400;">
  Laser Technologies Private Limited
    ...
    await transporter.sendMail({
      ...
      replyTo: email,
      subject: `MAIN-WEBSITE [Event Registration] ${event_name} — ${name}`,
      html: adminHtml,
    });
